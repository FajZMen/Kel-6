...
def load_user_data():
    if os.path.exists(EXCEL_FILE):
        ...
        for account_number, account_info in user_data.items():
            mutasi_data = account_info.get('mutasi', '')
            if isinstance(mutasi_data, str):
                account_info['mutasi'] = mutasi_data.split(';') if mutasi_data else []
            else:
                account_info['mutasi'] = []
        return user_data
    return {}
...
def save_user_data():
    try:
        ...
        # Ensure 'mutasi' is stored as a semicolon-separated string
        for account_number, account_info in user_data.items():
            if isinstance(account_info['mutasi'], list):
                account_info['mutasi'] = ';'.join(account_info['mutasi'])

        df = pd.DataFrame.from_dict(user_data, orient="index")
        df.to_excel(EXCEL_FILE, index_label="account_number", engine='openpyxl')

        # Restore `mutasi` as a list in session state
        for account_number, account_info in user_data.items():
            account_info['mutasi'] = account_info['mutasi'].split(';') if account_info['mutasi'] else []
...
def transaction_menu(account_number):
    ...
    if transaction_option == "Lihat Saldo":
        ...
        if st.button("Setor"):
            ...
            st.session_state.user_accounts[account_number]['mutasi'].append(f"Setoran: +Rp{deposit_amount} ({current_date})")
            save_user_data()
        ...
        if st.button("Transfer"):
            if transfer_account in st.session_state.user_accounts:
                if st.session_state.user_accounts[account_number]['saldo'] >= transfer_amount:
                    ...
                    st.session_state.user_accounts[account_number]['mutasi'].append(
                        f"Transfer keluar: -Rp{transfer_amount} ke {transfer_account} ({current_date})")
                    st.session_state.user_accounts[transfer_account]['mutasi'].append(
                        f"Transfer masuk: +Rp{transfer_amount} dari {account_number} ({current_date})")
...
faq_data = {
    ...
    "keamanan": "Pastikan untuk menjaga keamanan rekening Anda! Jangan bagikan PIN atau password kepada siapa pun, dan selalu logout setelah selesai.",
    ...
    "pembuat" : "Pembuat program ini adalah Nathaniel Lemuel Chandra ",
    "lupa nomor rekening": "Jika Anda lupa nomor rekening, silakan hubungi layanan pelanggan di support@equitrustfinance.com atau +62 123 456 789.",
    "kartu atm": "untuk fitur kartu atm kami disini masih belum memiliki fitur",
}

# Fungsi pencocokan cerdas
def get_response(user_query):
    normalized_query = user_query.lower().strip()
    matched_keywords = get_close_matches(normalized_query, faq_data.keys(), n=1, cutoff=0.4)
